 ...
 *
 */
package org.guruha.commons;

import org.guruha.exceptions.InvalidDicePoolStringException;

/**
 * The DicePool class stores the elements of a dice pool and allow retrieve
 * them easily.
 * 
 * <p>
 * Any Dice set and its modifiers can be represented by a instance of this
 * class and also permit convert from/to a dice string representation 
 * (i.e. "2d+1", "3d"). It also allow a empty dice number to represent 
 * absolutes numbers, like a pure 1 point of damage without dice roll.
 * </p>
 *
...
public class DicePool {
	 ...
	 *
	 * <p>
	 * This is usefull when you need do make arithmetics with dices.
	 * </p>
	 *
	 */
	 ...
	 *
	 * <p>Most dice rolls on GURPS uses three dices.</p>
	 *
	 */
	public DicePool() {
		dices = DEFAULT_DICES;
	 ...
	 *
	 * <p>This is useful to create a quick dice only pool without ant 
	 * modifier.</p>
	 *
	 ...
	 *
	 * <p>
	 * Use this to create a complete dice pool with both dice number
	 * and modifiers. (i.e. 2d+2, 7d+7, etc).
	 * </p>
	 *
	 * @param dices The number of dices.
	 * @param modifiers The total modifiers roll.
	 *
	 */
	 ...
	 * <i>n</i><strong>d</strong>+<i>m</i>, where <i>n</i> is the number of dices 
	 * to roll and <i>m</i> is the modifier, a integer to be aplied to final 
	 * result.
	 * </p>
	 *
	 * @param dicePoolString A string with a valid dice pool format.
	 *
	...
	public DicePool(String dicePoolString) {
		this.dices = parseDicesFromString(dicePoolString);
		this.modifiers = parseModifiersFromString(dicePoolString);
	}
	...
	private Integer parseModifiersFromString(String dicePoolString) {
		...
		if (dicePoolString.matches(".*[dD].*")) {
			// We have a dice
			if(dicePoolString.matches(".*[+-].*")) {
				// We have a modifier
				return new Integer(dicePoolString.replaceAll(".*[dD](\\+){0,1}", ""));
			} else {
				return 0;
			}
		} else {
	...
	/**
	 * Verify if a given dice pool string is valid, returning true if yes 
	 * or false otherwise.
	 *
	 * @param dicePoolString A string representing a dice pool.
	 *
	 */
	public static boolean checkDicePoolString(String dicePoolString) {
		return dicePoolString.matches("(([0-9]+[dD]){0,1}([+-][0-9]+){0,1}|[0-9]+)");
	}

	/**
	 * Retrive the dice number integer from the pool.
	 *
	...
	/**
	 * Retrive the total modifiers integer from the pool.
	 *
