...
public class DicePool {
    ...
    private Integer parseDicesFromString(String dicePoolString) {
        if (!checkDicePoolString(dicePoolString)) throw new InvalidDicePoolStringException(
                dicePoolString);
    ...
    private Integer parseModifiersFromString(String dicePoolString) {
        ...
        if (dicePoolString.matches(".*[dD].*")) {
            ...
            return new Integer(
                    dicePoolString.replaceAll(".*[dD](\\+){0,1}", ""));
            else return 0;
        } else return new Integer(dicePoolString);
    }
    ...
    public String toString() {
        if (dices == 0 && modifiers == 0) return "0";
        
        // TODO: Use string builder to make this more efficient.
        String dicePoolString = "";
        
        if (dices != 0) dicePoolString = dicePoolString.concat(dices + "d");
        
        if (modifiers != 0) if (modifiers < 0) dicePoolString = dicePoolString
                .concat(modifiers.toString());
        else if (modifiers > 0) if (dices != 0) dicePoolString = dicePoolString
                .concat("+" + modifiers);
        else dicePoolString = dicePoolString.concat(modifiers.toString());
